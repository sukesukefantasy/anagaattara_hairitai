...
void main() {
    ...
    vec3 ambient_tint = u_ambient_color;
    ...
    // 光が届かないピクセルは描かない（既存 ambient の上に二重暗転しない）
    if (effective_darkness >= u_darkness_alpha - 0.0001) {
        discard;
    }

    vec3 ambient_dark = mix(ambient_tint, ambient_tint * 0.45, effective_darkness);
    outColor = vec4(ambient_dark * effective_darkness, effective_darkness);
}
