...
class ApartmentInteriorScene extends GameScene {
  ...
  Vector2? _playerPosition; // プレイヤーの現在位置（シーン内でのみ管理）
  Vector2? get playerPosition => _playerPosition; // getterを定義
  ...
  ApartmentInteriorScene({
    ...
    String? outdoorSceneIdFromSave, // 追加
        ...
        _outdoorSceneIdFromSave = outdoorSceneIdFromSave, // 初期化
        super();
  ...
  Future<void> initializeScene(dynamic data) async {
    // 背景の追加
    final interiorBackgrounds = backgroundDataMap['apartment_interior'];
    ...
    // 地面を追加
    final interiorGroundSprite = await Sprite.load('concrete_ground.png'); // TODO: 適切な地面スプライト
    _ground = Ground(
    ...
    final counterComponent = _InteriorCounter(
      sprite: await Sprite.load(
        ...
        srcSize: Vector2(32, 21), // TODO: 適切なsrcSize
      ),
      position: Vector2(
        _backgroundComponent!.position.x +
            ((1792 - 1504) * scale), // TODO: 背景画像内の相対位置を調整
    ...
    debugPrint('ApartmentInteriorScene _initialPlayerPosition: $_initialPlayerPosition');
    debugPrint('ApartmentInteriorScene _backgroundComponent size: ${_backgroundComponent?.size}');
  }
      ...
      if (!_isShowingExitAction) {
        GameUI.setInteractAction(() {
          ...
          if (outdoorSceneDefinitions != null) {
            final String buildingType = _buildingTypeFromSave ?? 'apartment'; // セーブデータからのタイプを優先
