...
class SushiInteriorScene extends GameScene {
  ...
  Future<void> initializeScene(dynamic data) async {
    // 背景の追加
    final interiorBackgrounds = backgroundDataMap['sushi_interior'];
    ...
    debugPrint(
      'SushiInteriorScene initializeScene start for sushi: ${_enteredBuilding?.toString()}',
    );
    ...
    final counterComponent = _InteriorCounter(
      sprite: await Sprite.load(
        'CITY_MEGA.png',
        srcPosition: Vector2(786, 835),
        srcSize: Vector2(94, 29),
      ),
      position: Vector2(
    ...
    debugPrint(
      'SushiInteriorScene initializeScene complete for sushi: ${_enteredBuilding?.toString()}',
    );
    debugPrint(
      ...
      if (!_isShowingExitAction) {
        GameUI.setInteractAction(() {
          ...
          if (outdoorSceneDefinitions != null) {
            final String buildingType = _buildingTypeFromSave ?? 'sushi'; // セーブデータからのタイプを優先
            buildingDefinition = outdoorSceneDefinitions[buildingType];
          }

          if (buildingDefinition == null) {
            // 指定された屋外シーンIDの定義がない、または指定された建物タイプが見つからない場合、汎用的な出口位置を設定
            debugPrint('Warning: Building definition for ${_buildingTypeFromSave ?? 'sushi'} not found in $targetOutdoorSceneId. Placing player at a default exit point in $targetOutdoorSceneId.');
...
class _InteriorCounter extends SpriteComponent
  ...
  Future<void> onLoad() async {
    ...
    if (game.player != null) {
      ...
      debugPrint('SushiInteriorScene: Player priority set to 10.');
    }
    add(
      RectangleHitbox(
        size: size,
  ...
  void onCollisionStart(Set<Vector2> intersectionPoints, PositionComponent other) {
    ...
    if (other is Player) {
      debugPrint('Player collided with SushiCounter!');
      GameUI.setInteractAction(() {
        debugPrint('寿司屋のカウンターとインタラクトしました。');
        // TODO: 寿司屋のインタラクト処理
      }, Icons.ramen_dining); // 寿司屋のアイコン (適切なものがないのでラーメンで代用)
    }
  }
