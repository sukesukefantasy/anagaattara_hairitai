...
class BurgerStoreInteriorScene extends GameScene {
  ...
  Future<void> initializeScene(dynamic data) async {
    // 背景の追加
    final interiorBackgrounds = backgroundDataMap['burger_store_interior'];
    ...
    debugPrint(
      'BurgerStoreInteriorScene initializeScene start for burger_store: ${_enteredBuilding?.toString()}',
    );
    ...
    final counterComponent = _InteriorCounter(
      sprite: await Sprite.load(
        'CITY_MEGA.png',
        srcPosition: Vector2(1696, 923),
        srcSize: Vector2(72, 21),
      ),
      position: Vector2(
      ...
      size: Vector2(72 * scale, 21 * scale),
    )..priority = 40;
    ...
    // await add(game.player!); // BurgerStoreInteriorSceneの子としてプレイヤーを追加
    ...
    debugPrint(
      'BurgerStoreInteriorScene initializeScene complete for burger_store: ${_enteredBuilding?.toString()}',
    );
    debugPrint(
      ...
      if (!_isShowingExitAction) {
        GameUI.setInteractAction(() {
          ...
          if (outdoorSceneDefinitions != null) {
            final String buildingType = _buildingTypeFromSave ?? 'burger_store'; // セーブデータからのタイプを優先
            buildingDefinition = outdoorSceneDefinitions[buildingType];
          }

          if (buildingDefinition == null) {
            // 指定された屋外シーンIDの定義がない、または指定された建物タイプが見つからない場合、汎用的な出口位置を設定
            debugPrint('Warning: Building definition for ${_buildingTypeFromSave ?? 'burger_store'} not found in $targetOutdoorSceneId. Placing player at a default exit point in $targetOutdoorSceneId.');
...
class _InteriorCounter extends SpriteComponent
  ...
  Future<void> onLoad() async {
    ...
    if (game.player != null) {
      ...
      debugPrint('BurgerStoreInteriorScene: Player priority set to 10.');
    }
    add(
      RectangleHitbox(
        size: size,
  ...
  void onCollisionStart(Set<Vector2> intersectionPoints, PositionComponent other) {
    ...
    if (other is Player) {
      debugPrint('Player collided with BurgerStoreCounter!');
      GameUI.setInteractAction(() {
        debugPrint('ハンバーガー店のカウンターとインタラクトしました。');
        // TODO: ハンバーガー店のインタラクト処理
      }, Icons.fastfood); // ハンバーガーのアイコン
    }
  }
