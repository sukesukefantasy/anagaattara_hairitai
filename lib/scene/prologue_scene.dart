...
class PrologueScene extends AbstractOutdoorScene {
  ...
  void update(double dt) {
    ...
    final double transitionFactor = ((playerX - transitionStart) /
    ...
    if (ground != null) {
      ...
      final r =
          lerpDouble(
            startColor.red.toDouble(),
            endColor.red.toDouble(),
            transitionFactor,
          )!.toInt();
      final g =
          lerpDouble(
            startColor.green.toDouble(),
            endColor.green.toDouble(),
            transitionFactor,
          )!.toInt();
      final b =
          lerpDouble(
            startColor.blue.toDouble(),
            endColor.blue.toDouble(),
            transitionFactor,
          )!.toInt();
      final a =
          lerpDouble(
            startColor.alpha.toDouble(),
            endColor.alpha.toDouble(),
            transitionFactor,
          )!.toInt();
  ...
  void spawnGhostEchoes() {
    ...
    for (final routeId in ordered) {
      ...
      final color = _prologueGhostEchoPalette[routeId] ?? Colors.white70;
      final ghost = GhostEcho(
        routeId: routeId,
        color: color,
        position: Vector2(
          -400 - (i * 150.0),
          game.initialGameCanvasSize.y - 32,
        ),
      );
      ghost.priority = 35;
      add(ghost);
    ...
    switch (state.youngUncleDialogueStep) {
      case 0:
        game.windowManager.showDialog(
          [
            "[${npc.name}]",
            "「……綺麗な空だ。なんだか、ずっと昔からこうして空を眺めていたような気がするんだ。」",
