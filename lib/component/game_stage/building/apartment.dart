...
import 'dart:math';
import '../../../main.dart';
import '../../../scene/apartment_interior_scene.dart'; // ApartmentInteriorSceneをインポート
...
class Apartment extends Building {
  ...
  Future<void> onLoad() async {
    ...
    final buildingBodySprite = await Sprite.load(
      'CITY_MEGA.png',
      srcPosition: Vector2(642, 1620),
      srcSize: Vector2(106, 220),
    );

    _doorClosedSprite = await Sprite.load(
      'CITY_MEGA.png',
      srcPosition: Vector2(687, 1810),
      srcSize: Vector2(18, 30),
    );

    _doorOpenSprite = await Sprite.load(
      'CITY_MEGA.png',
      srcPosition: Vector2(687, 1554),
      srcSize: Vector2(18, 30),
    );
    ...
    _doorSprite = SpriteComponent(
      ...
      position: Vector2((687 - 642) * 2, (1810 - 1620) * 2), // 相対位置を計算
  ...
  void _updateInteractAction() {
    if (_isPlayerNearDoor) {
      GameUI.setInteractAction(() {
        ...
        game.sceneManager.enterApartmentScene(this, initialPlayerPosition: null, outdoorSceneId: currentOutdoorSceneId); // outdoorSceneIdを渡す
      }, Icons.home);
    } else {
