...
import 'dart:math';
import '../../../main.dart';
import '../../../scene/sushi_interior_scene.dart'; // SushiInteriorSceneをインポート
import '../../../scene/abstract_outdoor_scene.dart'; // AbstractOutdoorSceneをインポート

class Sushi extends Building {
  ...
  Future<void> onLoad() async {
    ...
    final buildingBodySprite = await Sprite.load(
      'CITY_MEGA.png',
      srcPosition: Vector2(519, 1664),
      srcSize: Vector2(101, 176),
    );

    _doorClosedSprite = await Sprite.load(
      'CITY_MEGA.png',
      srcPosition: Vector2(559, 1810),
      srcSize: Vector2(34, 30),
    );

    _doorOpenSprite = await Sprite.load(
      'CITY_MEGA.png',
      srcPosition: Vector2(559, 1554),
      srcSize: Vector2(34, 30),
    );
    ...
    _doorSprite = SpriteComponent(
      ...
      position: Vector2((559 - 519) * 2, (1810 - 1664) * 2), // 相対位置を計算
  ...
  void _updateInteractAction() {
    if (_isPlayerNearDoor) {
      GameUI.setInteractAction(() {
        ...
        game.sceneManager.enterSushiScene(this, initialPlayerPosition: null, outdoorSceneId: currentOutdoorSceneId); // outdoorSceneIdを渡す
      }, Icons.restaurant);
    } else {
