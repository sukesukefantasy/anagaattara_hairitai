...
import 'dart:math';
import '../../../main.dart';
import '../../../scene/health_center_interior_scene.dart'; // HealthCenterInteriorSceneをインポート
import '../../../scene/abstract_outdoor_scene.dart'; // AbstractOutdoorSceneをインポート

class HealthCenter extends Building {
  ...
  }) : super(type: 'health_center'); // `type` を追加
  ...
  Future<void> onLoad() async {
    ...
    final buildingBodySprite = await Sprite.load(
      'CITY_MEGA.png',
      srcPosition: Vector2(104, 1688),
      srcSize: Vector2(160, 153),
    );

    _doorClosedSprite = await Sprite.load(
      'CITY_MEGA.png',
      srcPosition: Vector2(167, 1810),
      srcSize: Vector2(34, 30),
    );

    _doorOpenSprite = await Sprite.load(
      'CITY_MEGA.png',
      srcPosition: Vector2(167, 1554),
      srcSize: Vector2(34, 30),
    );
    ...
    _doorSprite = SpriteComponent(
      ...
      position: Vector2((167 - 104) * 2, (1810 - 1688) * 2), // 相対位置を計算
      size: Vector2(34 * 2, 30 * 2),
    );
    add(_doorSprite);
  ...
  void _updateInteractAction() {
    if (_isPlayerNearDoor) {
      GameUI.setInteractAction(() {
        ...
        game.sceneManager.enterHealthCenterScene(this, initialPlayerPosition: null, outdoorSceneId: currentOutdoorSceneId); // outdoorSceneIdを渡す
